<li data-id="<?=$image->id?>" class="js-list-group-item js-item<?=($image->act ? '' : ' disabled')?>">
    ...
    <a href="<?=URL::to('/')?>/admin/image/act/<?=$image->id?>" class="act img-checkbox <?=($image->act ? 'enabled' : 'disabled')?> js-ajax"></a>
    <a href="<?=URL::to('/')?>/admin/image/<?=$image->id?>/edit" class="edit js-ajax" data-target="#image-area">
        <img src="<?=URL::to('/')?>/assets/images/edit.png">
    </a>
    <a href="<?=URL::to('/')?>/admin/image/<?=$image->id?>" class="js-ajax delete">
